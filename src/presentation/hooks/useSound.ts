...
export function useSound() {
  ...
  const [gameBgmStyle, setGameBgmStyleState] = useState<GameBgmStyle>("tavern");
  ...
  const playPlaceMark = useCallback(() => soundService.play("placeMark"), []);
  ...
  const playChat = useCallback(() => soundService.play("chat"), []);
  ...
  const updateBgmVolume = useCallback((newVolume: number) => {
    ...
    localStorage.setItem("bgm_volume", String(newVolume));
  }, []);
